...
def finallyRetired(balance, expense, rate, max_years=200):
    ...
    # ---- Loop until funds are depleted or safety cap reached ----
    while balance > 0 and years < max_years:
        balance = balance * (1 + rate)  # apply interest
        balance = balance - expense     # apply withdrawal
        years += 1
